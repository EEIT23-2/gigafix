...
<template>
  <main class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn btn-primary" @click="openCreateModal">＋ 新增分店</button>
    </div>
      ...
      <table v-else class="table table-hover mb-0 align-middle">
        ...
        <tbody>
          <tr v-for="s in stores" :key="s.id">
            ...
            <td>{{ s.phone }}</td>
            <td class="text-end">
              <button class="btn btn-sm btn-outline-primary me-2" @click="openEditModal(s)">修改</button>
              <button class="btn btn-sm btn-outline-danger" @click="handleDelete(s)">刪除</button>
            </td>
          </tr>
          <tr v-if="stores.length === 0">
            <td colspan="5" class="text-center text-secondary py-4">目前沒有分店資料</td>
          </tr>
        </tbody>
      </table>
    </section>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title">{{ editingId === null ? "新增分店" : "修改分店" }}</h5>
          ...
          <div class="modal-body">
            ...
            <input v-model="form.address" type="text" class="form-control mb-2" :disabled="saving" />
