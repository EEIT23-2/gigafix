...
<template>
  <main class="container-fluid px-3 px-lg-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn btn-primary" @click="openCreateModal">＋ 新增技師</button>
    </div>

    <section class="card mb-4">
      <div class="card-body d-flex gap-3 align-items-center">
        <label class="form-label mb-0">依分店篩選：</label>
        <select v-model="filterStoreId" class="form-select" style="max-width: 220px" @change="fetchTechnicians">
    ...
    <div v-if="successMessage" class="alert alert-success alert-dismissible" role="alert">
      {{ successMessage }}
      <button class="btn-close" type="button" @click="successMessage = ''"></button>
    </div>
      ...
      <table v-else class="table table-hover mb-0 align-middle">
        ...
        <tbody>
          <tr v-for="t in technicians" :key="t.id">
            ...
            <td>{{ t.storeName }}</td>
            <td class="text-end">
              <button class="btn btn-sm btn-outline-primary me-2" @click="openEditModal(t)">修改</button>
              <button class="btn btn-sm btn-outline-danger" @click="handleDelete(t)">刪除</button>
            </td>
          </tr>
          <tr v-if="technicians.length === 0">
            <td colspan="5" class="text-center text-secondary py-4">目前沒有技師資料</td>
          </tr>
        </tbody>
      </table>
    </section>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title">{{ editingId === null ? "新增技師" : "修改技師" }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <label class="form-label">姓名</label>
            <input v-model="form.name" type="text" class="form-control mb-2" :disabled="saving" />

            <label class="form-label">電話</label>
            <input v-model="form.phone" type="text" class="form-control mb-2" :disabled="saving" />

            <label class="form-label">分店</label>
            <select v-model="form.storeId" class="form-select mb-2" :disabled="saving">
              <option value="">請選擇分店</option>
              <option v-for="s in stores" :key="s.id" :value="s.id">{{ s.name }}</option>
            </select>

            <p v-if="formError" class="text-danger mt-2 mb-0">{{ formError }}</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" :disabled="saving">取消</button>
