...
watch([regAddressCity, regAddressDistrict, regAddressDetail], () =>
  checkRegisterError(),
);
...
const sendForgotPasswordOtp = async () => {
  ...
  try {
    await axios.post("/api/gigafix/members/forgot-password/otp", {
      email: fpEmail.value,
    });
    alert("驗證碼已寄出，請至信箱查收(5分鐘內有效)");
      ...
      <div class="header-inner">
        ...
        <nav class="main-nav">
          ...
          <ul class="nav-list">
            ...
            <router-link class="nav-item" to="/gigafix/mall"
              >二手手機 ▾</router-link
            >
            <router-link class="nav-item">維修手機 ▾</router-link>
            <router-link class="nav-item" to="/gigafix/forum"
