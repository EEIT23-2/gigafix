//台灣各縣市及所屬行政區資料，給前端註冊/修改資料表單的地址下拉選單用
//只有member(前台會員中心)和admin(後台會員管理)的前端會用到，後端只接收前端把縣市+行政區+詳細地址組合好的完整字串，不需要這份資料
//縣市名稱統一用「台」不用「臺」，跟後台熱點圖用的taiwan-atlas地圖資料(COUNTYNAME)命名一致，兩邊比對名稱不用再做別名轉換
export const taiwanDistricts = [
	...
	{
		city: '苗栗縣',
		districts: ['苗栗市', '苑裡鎮', '通霄鎮', '竹南鎮', '頭份市', '後龍鎮', '卓蘭鎮', '大湖鄉', '公館鄉', '銅鑼鄉', '南庄鄉', '頭屋鄉', '三義鄉', '西湖鄉', '造橋鄉', '三灣鄉', '泰安鄉', '獅潭鄉']
	},
	{
		city: '彰化縣',
	...
	{
		city: '台東縣',
		districts: ['臺東市', '成功鎮', '關山鎮', '卑南鄉', '大武鄉', '太麻里鄉', '東河鄉', '長濱鄉', '鹿野鄉', '池上鄉', '綠島鄉', '蘭嶼鄉', '延平鄉', '海端鄉', '達仁鄉', '金峰鄉']
	},
	{
		city: '澎湖縣',
...
//依縣市名稱取得該縣市的行政區陣列，給下拉選單的第二層(行政區)連動第一層(縣市)用，找不到就回傳空陣列
export function getDistrictsByCity(city) {
	const found = taiwanDistricts.find(item => item.city === city)
	return found ? found.districts : []
}

//把完整地址字串拆成縣市/行政區/詳細地址三部分，開編輯彈窗要把既有會員地址帶回下拉選單時用
//找不到對應縣市，或縣市後面接的不是已知行政區，就整段當作詳細地址，不硬拆避免資料跑掉
export function parseTaiwanAddress(address) {
	if (!address) {
		return { city: '', district: '', detail: '' }
	}
	const matchedCity = taiwanDistricts.find(item => address.startsWith(item.city))
	if (!matchedCity) {
		return { city: '', district: '', detail: address }
	}
	const rest = address.slice(matchedCity.city.length)
	const matchedDistrict = matchedCity.districts.find(d => rest.startsWith(d))
	return {
		city: matchedCity.city,
		district: matchedDistrict || '',
		detail: matchedDistrict ? rest.slice(matchedDistrict.length) : rest
	}
}
