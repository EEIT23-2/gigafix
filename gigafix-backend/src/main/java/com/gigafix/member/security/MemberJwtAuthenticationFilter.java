    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null && jwtUtils.validateToken(token)) {
            ...
            try {
                ...
                // token簽章是合法的，但裡面的memberId在資料庫已經查無此人
                // （帳號被刪除、或資料庫被重建但瀏覽器還留著舊的登入cookie）
                // 不能讓例外炸穿到servlet層變成500，這裡當作未登入繼續放行，並清掉這個失效的cookie
                ResponseCookie expiredCookie = jwtUtils.createExpiredTokenCookie();
