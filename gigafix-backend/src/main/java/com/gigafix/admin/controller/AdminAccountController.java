...
public class AdminAccountController {
	...
	public ResponseEntity<AdminInfoDto> getMyInfo(Authentication authentication) {
		AdminUserDetails currentAdmin = SecurityUtils.getCurrentAdmin(authentication);
		// 直接從這次請求的登入認證裡抓登入資訊，從request在登入驗證時手動把authentication掛到SecurityContextHolder，spring
		// MVC就可以把authentication撈出來從參數取得
	...
	public ResponseEntity<AdminInfoDto> updateMeName(Authentication authentication,
			@Valid @RequestBody UpdateMeNameReq updateMeNameReq) {
		Integer userId = SecurityUtils.getCurrentAdmin(authentication).getId();
		return ResponseEntity.ok(accountService.updateupdateMyName(userId, updateMeNameReq.newName()));
	}

	// → 改自己密碼
	@PatchMapping("/me/password")
	public ResponseEntity<Void> updateMyPassword(Authentication authentication,
			@Valid @RequestBody UpdateOwnPasswordReq req) {
		Integer userId = SecurityUtils.getCurrentAdmin(authentication).getId();
