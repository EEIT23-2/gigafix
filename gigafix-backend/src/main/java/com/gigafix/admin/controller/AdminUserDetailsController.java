...
public class AdminUserDetailsController {
	...
	@PostMapping("/adminlogin") // 因為是前後端分離專案，前端回傳Json，spring
								// security的預設登入formLogin()不讀取Json，所以不寫Json轉換器的話只能自己寫login
	public ResponseEntity<AdminLoginResp> adminLogin(@Valid @RequestBody AdminLoginReq adminLoginReq,
			HttpServletRequest req, HttpServletResponse resp) {
		Authentication authentication;

		try {
			// Spring
			// security的方法會做登入認證，自動呼叫我自己在AdminUserDetailsService寫的loadUserByUsername() +
			// 密碼比對
			...
			authentication = authenticationManager.authenticate(
					new UsernamePasswordAuthenticationToken(adminLoginReq.adminName(), adminLoginReq.password()));
			// 回傳的Authentication裡面包含→Principal(可以拿到UserDetails物件)、Authorities(權限)、Credentials密碼、認證後結果
		} catch (InternalAuthenticationServiceException e) {
			// loadUserByUsername() 裡丟出的例外，被 Spring Security 包了一層，這裡解開
		...
		req.changeSessionId(); // 代替spring security透過formLogin時會做的抽換session
								// id，因為不想寫SessionAuthenticationStrategy
