...
public class SupportChatServiceImpl implements SupportChatService {
	...
	// 要等到有人按下送出、Groq 回 401 才會發現——那時前端只看得到一個沒有前因後果的 502。
	// 這裡在啟動階段就擋下來，讓部署漏設/貼錯金鑰直接反映在容器起不來，而不是變成單一模組的神祕故障。
	// 同時印出長度（不印值）方便比對雲端拿到的金鑰有沒有被截斷或夾帶空白。
	@PostConstruct
	void validateGroqConfig() {
		if (groqApiKey == null || groqApiKey.isBlank()) {
			throw new IllegalStateException(
					"環境變數 GIGAFIX_SUPPORT_CHAT_GROQ 是空的，AI 客服無法運作，請設定有效的 Groq API Key");
		}
		log.info("AI 客服 Groq 設定載入完成 model={} baseUrl={} apiKeyLength={}",
				groqModel, groqApiBaseUrl, groqApiKey.length());
	}
	...
	public ChatResponse chat(Long memberId, ChatRequest request) {
		...
		try {
			...
			// Groq 有回應、但不是 2xx：401 金鑰不對、429 額度用完、404 模型下架都長這樣，
			// 回應 body 會寫明原因。這裡一定要記下來，不然部署環境出事時外面只看得到一個沒有線索的 502。
			// 注意：只記狀態碼與 body，絕對不要把 groqApiKey 印進日誌
			log.error("呼叫 Groq 失敗，上游回應 status={} body={}", e.getStatusCode(), e.getResponseBodyAsString());
			throw SupportChatException.upstreamError("客服服務暫時無法使用，請稍後再試");
		} catch (RestClientException e) {
			// 連線被拒、DNS 解不到、逾時、回應解析失敗等「根本沒拿到完整回應」的情況
			log.error("呼叫 Groq 失敗，無法取得上游回應", e);
			throw SupportChatException.upstreamError("客服服務暫時無法使用，請稍後再試");
		}
