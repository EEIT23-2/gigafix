...
- **其他API串接**：reCAPTCHA、Google 一鍵登入（Google Client 後端/Google-login 前端）、Cloudinary（圖片上傳/管理）、綠界 ECPay（線上付款）、Google Maps（維修分店定位）、Spring Mail（寄送 Email 通知，如 OTP、忘記密碼、訂單/維修/回收通知）、jjwt、echarts、SweetAlert2、bootstrap/bootstrap-icon、taiwan-atlas、groq
- **版本控制**：Git、GitHub
